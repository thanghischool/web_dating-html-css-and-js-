...
function loadData() {
  var content = data[0]
    .map(function (value) {
      return `<div class="item1" style="background: #FFF" data_id="${
        value.id
      }" onclick="checked(${value.id - 1},0,'item1')">
        <img src="${value.icon}">
        ${value.name}
    </div>`;
    })
  ...
  content = data[1]
    .map(function (value) {
      return `<div class="item2" style="background: #FFF" data_id="${
        value.id
      }" onclick="checked(${value.id - 1},1,'item2')">
        <img src="${value.icon}">
        ${value.name}
    </div>`;
    })
